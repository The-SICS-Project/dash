...
const Navbar=()=> {
    ...
    const GetName = async() => {
        ...
        let res= await fetch('https://sics-backend.azurewebsites.net/account/hello', options);
        if(res.status===200) {
            ...
            console.log("firstname fetch error")
        }
        const Ticketoptions = {
            method: 'GET',
            headers: {
       ...
       let response=await fetch('https://sics-backend.azurewebsites.net/tickets/stats', Ticketoptions)
           if(response.status===200){
               const ticketdetails=await response.json()
               setTicketcounter(ticketdetails.tickets)
    ...
    <div className={"searchbar"}>
        ...
        <div className="profile_window">
            <button className="new_ticket" >
                <CustomLink to="/ticket">+ New Ticket</CustomLink>
            </button>
